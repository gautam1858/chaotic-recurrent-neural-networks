...
import gc
from nn import NN
from chaotic_network import ChaoticRNN
...
with tf.Graph().as_default():
    ...
    for nn in range(sum_no_of_batches*FLAGS.num_epochs):
        ...
        if len(batch)<1:
            continue
        x1_batch,x2_batch, y_batch = list(zip(*batch)) # TODO: may not need list here
        if len(y_batch)<1:
            continue
        ...
        if current_step % FLAGS.evaluate_every == 0:
            ...
            for db in dev_batches:
                if len(db)<1:
                    continue
                x1_dev_b,x2_dev_b,y_dev_b = list(zip(*db)) # TODO: may not need list here
                if len(y_dev_b)<1:
                    continue
